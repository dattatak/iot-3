...
function parseFeed(el_id, channel_id, field_number) {
  ...
  $.getJSON('https://api.thingspeak.com/channels/' + channel_id + '/fields/' + field_number + '.json?offset=0&round=2&results=1', function(data) {
    ...
    var start = Date(data.feeds[0]['created_at']);
    a_el.setAttributeNS(linkns, 'title', 'Updated' + start);
    a_el.setAttributeNS(linkns, 'href', 'https://thingspeak.com/channels/' + channel_id + '/charts/' + field_number + '?results=20');
